...
@Component({
  selector: 'app-edit-offday-vacation-request-modal',
  templateUrl: './edit-offday-vacation-request-modal.component.html',
  styleUrls: ['./edit-offday-vacation-request-modal.component.scss'],
})
export class EditOffdayVacationRequestModalComponent  implements OnInit {
  ...
  ngOnInit() {
    this.offdayRequest = this.navParams.get('offdayRes');

      var note = {
        start : this.offdayRequest.start ?? '',
        end : this.offdayRequest.end ?? '',
        notes: this.offdayRequest.notes ?? ''
      }
  ...
  async saveForm() {
    if (this.form.valid) {
      ...
      await this.putAbsense(this.start, this.end, this.notes, this.status, this.companyId, this.offdayRequest.id );
  ...
  async putAbsense(start: string, end: string, notes: string, status: string, companyId: string, id: string ){
    try{
      await this.offdaysService.putOffdayRequests(start, end, notes, status, companyId, id);
    }catch (error){
      await this.functions.presentToast(`Erro ao editar pedido`, `danger`);
    }
  }
  ...
  async delete() {
    const alert = await this.alertCtrl.create({
      header: 'Confirmação',
      message: 'Tem certeza de que deseja apagar este pedido? Esta ação não pode ser desfeita.',
      buttons: [
        {
          text: 'Cancelar',
          role: 'cancel',
        ...
        },
        {
          ...
          handler: async () => {
            try {
              const loading = await this.functions.showLoading();
              await this.deleteAbsense(this.offdayRequest.id);
              loading.dismiss();
  ...
  async deleteAbsense(id: string){
    try{
      await this.offdaysService.deleteOffdayRequests(id);
    }catch (error){
